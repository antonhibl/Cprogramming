...
int main()
{
  ...
  // If generating a random data file
  if(genrand == UPPER_YES || genrand == LOWER_YES)
  {
	  // Download the random values
	  downloadRandomValues(input_filename);
	  // Upload the data and assign the output to item count
	  count = uploadData( input_filename, num_array);
  }
  // If Not generating a random data file
  if(genrand == UPPER_NO || genrand == LOWER_NO)
  {
	  // If file exists
	  if(openInputFile( input_filename ))
	  { 
		  count = uploadData( input_filename, num_array );
	  }
	  // If it doesn't exist
	  else
	  {
		  ...
		  printf("ERROR: Data file not found. Program aborted.\n\n");
  ...
  // If input is incorrect from user
  if(genrand != UPPER_YES && genrand != UPPER_NO && 
		  genrand != LOWER_YES && genrand != LOWER_NO)
  {
	  // Print Error in input
	  printf("ERROR: Incorrect input. Please enter yY/nN.\n\n");
	  // print End of Program
	  printf("END OF PROGRAM.");
	  // Return Error
	  return 1;
