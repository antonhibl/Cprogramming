...
/* David's Comments:
 I wouldnt make any changes to the code. It runs and runs well.
 If I had to recommend any change it would be to put the functions in
 alphabetical order but thats being super picky and does not pertain to
 the code itself. The functions were easy to follow and gave the expected results.
*/

/* My Comments to David:
 double check function return types and what values you actually return,
 also wouldnt hurt to double check loop logic there was one place where I
 saw it decrementing where it should have incremented which caused an 
 infinite loop. I overall think the code was very clearly documented and 
 showed they had a clear approach and understanding of the problem. More
 time debugging and I am sure they would have a calculable approach. Overall
 very readable code and well defined behaviours for the most part.
*/
...
int look_towards(int direction, int row, int col)
{
    // Initialize local variables
     // forward character
    register char forward_char;
     // forward tile value
    register int tile_value;
...
int get_best_move(int row, int col)
{
    // Initialize local variables
     // tile values
    register int east_tile, south_tile, north_tile, west_tile;
     // best choice value
    register int best_choice;
     // x for indexing
    register int x;
     // best move flag
    register bool best_flag = false;
...
void runner_solve(void) {
  ...
  register bool win_flag = false;
    // current character
  register char current_char;
    // moved flag starts out unset
  register bool moved_flag = false;
    // best move direction
  register int best_move;
